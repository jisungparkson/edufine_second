...
class BrowserManager:
    ...
    def reset_connection_for_new_service(self):
        """
        서비스 간 전환 시 Playwright 연결을 안전하게 재설정합니다.
        기존 연결을 종료하고 다음 get_page() 호출 시 새 브라우저 인스턴스를 강제 생성합니다.
        """
        try:
            print("서비스 전환을 위해 브라우저 연결을 재설정합니다...")
            
            if self.browser and self.browser.is_connected():
                print("기존 브라우저 연결을 종료합니다.")
                self.browser.close()
            
            # 브라우저와 페이지 객체를 None으로 초기화하여 새로운 인스턴스 강제 생성
            self.browser = None
            self.page = None
            
            print("브라우저 연결 재설정이 완료되었습니다.")
            
        except Exception as e:
            print(f"브라우저 연결 재설정 중 오류 발생: {e}")
            # 오류 발생 시에도 객체들을 None으로 초기화
            self.browser = None
            self.page = None
...
def navigate_to_neis(app_instance):
    """
    상황 인식 나이스 접속 함수:
    현재 페이지 상태를 판단하여 적절한 전략으로 나이스에 접속
    """
    try:
        print("=== 나이스 접속 상황 분석 및 전환 시작 ===")
        ...
        try:
            ...
            if current_page and not current_page.is_closed():
                ...
                print("현재 페이지가 없거나 닫혀있습니다.")
        except Exception as e:
            print(f"현재 페이지 확인 중 오류: {e}")
        
        # 2단계: 세션 전환 감지 - K-에듀파인에서 나이스로 전환하는 경우
        if current_url and 'klef.jbe.go.kr' in current_url:
            print("🔄 K-에듀파인에서 나이스로 서비스 전환을 감지했습니다.")
            messagebox.showinfo("서비스 전환", "나이스 접속을 위해 새로운 브라우저를 시작합니다.\n\n"
                                            "K-에듀파인과 나이스는 독립적인 세션을 사용하므로\n"
                                            "안전한 전환을 위해 브라우저 연결을 재설정합니다.")
        ...
        # 3단계: 이미 나이스에 있는 경우
            ...
            print("✓ 이미 나이스에 접속되어 있습니다.")
            messagebox.showinfo("완료", "이미 나이스에 접속되어 있습니다.")
            return
        
        # 4단계: 업무포털이나 기타 상황 - 기존 마스터 내비게이터 사용
        else:
            print("🚀 표준 나이스 접속 절차를 시작합니다...")
...
def navigate_to_neis_only():
    """나이스 메인 페이지로 이동 (하위 호환성을 위한 래퍼 함수)"""
...
def _perform_neis_login_workflow(app_instance) -> Page:
    """
    나이스 전용 로그인 워크플로우:
    나이스 URL로 이동 → 로그인 페이지 리디렉션 대기 → 사용자 수동 로그인 대기 → 나이스 페이지 복귀 대기
    """
    try:
        print("=== 나이스 전용 로그인 워크플로우 시작 ===")
...
def _perform_edufine_login_workflow(app_instance) -> Page:
    """
    K-에듀파인 전용 로그인 워크플로우:
    에듀파인 URL로 이동 → 로그인 페이지 리디렉션 대기 → 사용자 수동 로그인 대기 → 에듀파인 페이지 복귀 대기
    """
    try:
        print("=== K-에듀파인 전용 로그인 워크플로우 시작 ===")
        ...
        print(f"K-에듀파인 로그인 워크플로우 중 오류: {e}")
        raise
...
def navigate_to_edufine(app_instance):
    """
    상황 인식 K-에듀파인 접속 함수:
    현재 페이지 상태를 판단하여 적절한 전략으로 K-에듀파인에 접속
    """
    try:
        print("=== K-에듀파인 접속 상황 분석 및 전환 시작 ===")
        ...
        # 2단계: 세션 전환 감지 - 나이스에서 K-에듀파인으로 전환하는 경우
        if current_url and 'neis.go.kr' in current_url:
            print("🔄 나이스에서 K-에듀파인으로 서비스 전환을 감지했습니다.")
            messagebox.showinfo("서비스 전환", "K-에듀파인 접속을 위해 새로운 브라우저를 시작합니다.\n\n"
                                            "나이스와 K-에듀파인은 독립적인 세션을 사용하므로\n"
                                            "안전한 전환을 위해 브라우저 연결을 재설정합니다.")
            
            # 브라우저 연결 리셋
            browser_manager.reset_connection_for_new_service()
            
            # K-에듀파인 전용 로그인 워크플로우 실행
            page = _perform_edufine_login_workflow(app_instance)
            messagebox.showinfo("완료", "K-에듀파인 접속이 완료되었습니다.")
            return
        
        # 3단계: 이미 K-에듀파인에 있는 경우
        elif current_url and 'klef.jbe.go.kr' in current_url:
            print("✓ 이미 K-에듀파인에 접속되어 있습니다.")
            messagebox.showinfo("완료", "이미 K-에듀파인에 접속되어 있습니다.")
            return
        
        # 4단계: 업무포털이나 기타 상황 - 기존 마스터 내비게이터 사용
        else:
            print("🚀 표준 K-에듀파인 접속 절차를 시작합니다...")
